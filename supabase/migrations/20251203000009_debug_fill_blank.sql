-- Add comprehensive debugging for fill-in-blank scoring
-- This will help us see exactly what's being compared
      ...
      CASE v_question.question_type
        ...
        WHEN 'fill_in_blank' THEN
          ...
          IF jsonb_typeof(v_user_answer) = 'array' AND v_q_options IS NOT NULL AND jsonb_typeof(v_q_options) = 'array' THEN
             ...
             -- Debug: Log the full user answer array and options
             RAISE NOTICE 'Fill-in-blank question: %', v_question.id;
             RAISE NOTICE 'User answer array: %', v_user_answer;
             RAISE NOTICE 'Options (blank definitions): %', v_q_options;
             RAISE NOTICE 'Total blanks: %', v_total_blanks;
             ...
             LOOP
               ...
               IF jsonb_typeof(v_blank_def) = 'string' THEN
                 ...
                 EXCEPTION WHEN OTHERS THEN
                   RAISE NOTICE 'Could not parse blank definition at index %: %', v_array_index, v_blank_def;
               ...
               RAISE NOTICE 'Blank[%] - User answer: "%" | Accepted answers: %', v_array_index, v_blank_user_val, v_accepted_answers;
               ...
               IF v_blank_user_val IS NOT NULL AND jsonb_typeof(v_accepted_answers) = 'array' THEN
                 ...
                 LOOP
                   RAISE NOTICE 'Comparing: "%"(user) vs "%"(accepted) -> lowered: "%" vs "%"', 
                     v_blank_user_val, v_accepted_answer, 
                     LOWER(TRIM(v_blank_user_val)), LOWER(TRIM(v_accepted_answer));
               ...
               IF v_blank_correct THEN
                 ...
                 RAISE NOTICE 'Blank[%] marked CORRECT', v_array_index;
               ELSE
                 RAISE NOTICE 'Blank[%] marked INCORRECT', v_array_index;
               END IF;

               v_array_index := v_array_index + 1;
             END LOOP;

             RAISE NOTICE 'Final: % correct out of % blanks', v_correct_blanks, v_total_blanks;

             IF v_correct_blanks = v_total_blanks AND v_total_blanks > 0 THEN
               v_is_correct := TRUE;
               v_question_score := v_points;
               RAISE NOTICE 'ALL BLANKS CORRECT - Awarding % points', v_points;
             ELSE
               v_is_correct := FALSE;
               v_question_score := 0;
               RAISE NOTICE 'NOT ALL BLANKS CORRECT - Awarding 0 points';
             END IF;
          END IF;
      END CASE;
    END IF;
