-- Fix fill-in-blank indexing to properly match user answers with accepted answers
-- The issue is that blank definitions might have explicit 'index' fields, but user answers
-- are stored as arrays in sequential order [0, 1, 2, ...]
...
BEGIN
  ...
  LOOP
      ...
      CASE v_question.question_type
        ...
        WHEN 'essay' THEN
          ...
          IF v_question.correct_answer IS NOT NULL AND LENGTH(TRIM(v_question.correct_answer)) > 0 THEN
            ...
            END IF;
          ELSE
            -- If no expected answer is defined, essay requires manual grading - award 0 for now
            v_is_correct := FALSE;
            v_question_score := 0;
          END IF;
        ...
        WHEN 'checkbox', 'multiple_answers' THEN
          ...
          ELSE
             ...
             IF jsonb_typeof(v_user_answer) = 'array' THEN
                 ...
                 -- Award full points (v_points) if exact match, otherwise 0.
                 ...
                 END IF;
             ELSE
               -- User didn't provide an array answer for checkbox question
               v_question_score := 0;
               v_is_correct := FALSE;
             END IF;
          END IF;
        ...
        WHEN 'fill_in_blank' THEN
          -- STRICT BINARY SCORING: ALL blanks must be correct to earn ANY points
          -- User answers are stored as a simple array: ["answer1", "answer2", ...]
          -- Blank definitions contain accepted_answers for each blank position
          ...
          IF v_q_options IS NOT NULL AND jsonb_typeof(v_q_options) = 'array' THEN
             ...
             -- Loop through each blank definition in order
             ...
             LOOP
               ...
               IF jsonb_typeof(v_blank_def) = 'string' THEN
                 ...
                 EXCEPTION WHEN OTHERS THEN
                   -- If it's not valid JSON, skip this blank
                   v_array_index := v_array_index + 1;
                   CONTINUE;
                 END;
               END IF;

               -- Try to get the explicit index from the definition, otherwise use array position
               BEGIN
               ...
               -- Get user answer for this blank using the sequential array index
               -- User answers are always stored as ["ans0", "ans1", "ans2"] regardless of explicit indices
               IF jsonb_typeof(v_user_answer) = 'array' THEN
                 -- Use array position, not the 'index' field from the definition
                 BEGIN
               ...
               IF v_blank_user_val IS NOT NULL AND jsonb_typeof(v_accepted_answers) = 'array' THEN
                 -- Check against accepted answers (case insensitive, trimmed)
             ...
             -- Must get ALL blanks correct to earn ANY points
             -- If even ONE blank is wrong, score = 0
    ...
    -- Ensure question score never exceeds the question's point value
    IF v_question_score > v_points THEN
      v_question_score := v_points;
    END IF;

    -- Add question score to total (each question contributes at most once)
    v_total_score := v_total_score + v_question_score;
  END LOOP;

  -- CRITICAL FIX: Enforce final clamp to ensure score never exceeds max_score
  -- This handles any edge cases or data inconsistencies
