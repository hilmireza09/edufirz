-- Fix: Add explicit search_path and schema-qualify all object references in submit_quiz_attempt
-- This prevents ambiguous object resolution and security issues from mutable search_path
...
DECLARE
  ...
  v_chk_opt TEXT;
BEGIN
  -- Get attempt details (schema-qualified)
  ...
  -- Loop through questions (schema-qualified)
  FOR v_question IN
    SELECT * FROM public.quiz_questions WHERE quiz_id = v_quiz_id
  LOOP
             ...
             LOOP
               ...
               IF jsonb_typeof(v_blank_def) = 'string' THEN
                 ...
                 EXCEPTION WHEN OTHERS THEN
                   -- Ignore if not valid json, keep as is
                 END;
               END IF;
  ...
  -- Update the attempt with final score (schema-qualified)
...
-- Ensure proper permissions (revoke from PUBLIC for security, grant to authenticated)
...
'Grades a quiz attempt and updates the score. Uses explicit search_path and schema-qualified references to prevent ambiguous object resolution and security issues.';
