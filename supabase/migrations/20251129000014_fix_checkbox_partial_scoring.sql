-- Fix checkbox scoring to include partial credit
        ...
        WHEN 'checkbox', 'multiple_answers' THEN
          IF v_q_correct_answers IS NOT NULL AND jsonb_typeof(v_q_correct_answers) = 'array' AND jsonb_typeof(v_user_answer) = 'array' THEN
             ...
             IF v_chk_total_correct > 0 THEN
               ...
               LOOP
                 IF v_q_correct_answers @> to_jsonb(v_chk_opt) THEN
               ...
               -- Calculate score: (Correct - Incorrect) / Total * Points
               -- Ensure non-negative
               -- Example: 2 correct options. User selects 2 correct, 0 incorrect -> (2-0)/2 = 100%
               -- Example: 2 correct options. User selects 1 correct, 0 incorrect -> (1-0)/2 = 50%
               -- Example: 2 correct options. User selects 2 correct, 1 incorrect -> (2-1)/2 = 50%
               -- Example: 2 correct options. User selects 1 correct, 1 incorrect -> (1-1)/2 = 0%
               
               v_question_score := GREATEST(0, (v_chk_user_correct - v_chk_user_incorrect) / v_chk_total_correct) * v_points;
               
               -- Mark as fully correct only if perfect score
